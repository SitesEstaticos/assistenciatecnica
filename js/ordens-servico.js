...
async function viewOrderDetails(ordemId) {
    ...
    try {
        ...
        // DETALHES PRINCIPAIS
        document.getElementById('detailNumber').textContent = ordem.numero_os || 'N/A';
        document.getElementById('detailClient').textContent = cliente?.nome || 'N/A';

        const tipo = equipamento?.tipo ? `[${equipamento.tipo}] ` : '';
        const idUnico = equipamento?.numero_serie || equipamento?.mac_address || 'S/N';
        document.getElementById('detailEquipment').textContent = 
            `${tipo}${equipamento?.marca || ''} ${equipamento?.modelo || ''} (${idUnico})`.trim();

        document.getElementById('detailStatus').textContent = getStatusLabel(ordem.status);
        document.getElementById('detailDate').textContent = formatDate(ordem.data_recebimento);
        document.getElementById('detailTechnician').textContent = ordem.tecnico_responsavel || 'N/A';
        
        // Busca as peças registradas na OS para apresentar o total real no modal
        let totalPecas = 0;
        try {
            const pecasOS = await db.getPecasByOrdem(ordemId);
            if (pecasOS && pecasOS.length > 0) {
                totalPecas = pecasOS.reduce((acc, p) => acc + (Number(p.quantidade || 0) * Number(p.valor_unitario || 0)), 0);
            }
        } catch (err) {
            Logger.error('Erro ao somar peças para exibição de detalhes', err);
        }

        const valorTotalExibicao = Number(ordem.valor_servico || 0) + totalPecas;
        document.getElementById('detailValue').textContent = formatCurrency(valorTotalExibicao);

        document.getElementById('detailProblem').textContent = ordem.problema_relatado || 'N/A';
        document.getElementById('detailDiagnosis').textContent = ordem.diagnostico_tecnico || 'N/A';
        document.getElementById('detailServices').textContent = ordem.servicos_realizados || 'N/A';
...
async function loadPartsUsed(ordemId) {
    ...
    if (!pecas || pecas.length === 0) {
        partsTable.innerHTML = '<tr><td colspan="4" class="text-center">Nenhuma peça registrada</td></tr>';
        return;
    }
...
async function loadOrderImages(ordemId) {
    ...
    if (!ordem) {
        gallery.innerHTML = '<p class="text-center">Ordem de serviço não encontrada</p>';
        return;
    }
    ...
    (imagensDaOs || []).forEach(img => {
        if (!img?.id || !img?.url_imagem) return;
...
async function loadOrderTimeline(ordemId) {
    ...
    historico.forEach(h => {
        ...
        const timelineDescription = h.descricao || `Status alterado para <strong>${statusLabel}</strong>`;

        div.innerHTML = `
            <div class="timeline-date">${formatDate(eventDate)}</div>
            <div class="timeline-content">${timelineDescription}</div>
        `;
...
async function loadOrdensServico() {
    try {
        ...
        await renderOrdensTable(allOrdensServico);
...
async function loadClientes() {
    try {
        ...
        populateClientDropdown(allClientes);
    } catch (error) {
...
async function loadEquipamentos() {
    try {
        ...
        populateEquipmentDropdown(allEquipamentos);
    } catch (error) {
        Logger.error('Error loading equipamentos', error);
    }
}

function populateClientDropdown(clientesList, selectedId = '') {
...
function populateEquipmentDropdown(equipamentosList, selectedId = '') {
    ...
    equipamentosList.forEach(equip => {
        const option = document.createElement('option');
        option.value = equip.id;

        const tipo = equip.tipo ? `[${equip.tipo}] ` : '';
        const identificador = equip.numero_serie || equip.mac_address || 'S/N';
        option.textContent = `${tipo}${equip.marca || ''} ${equip.modelo || ''} (${identificador})`.trim();

        if (String(equip.id) === String(selectedId)) {
            option.selected = true;
        }
...
async function renderOrdensTable(ordens) {
    const tableBody = document.getElementById('ordersTable');
    tableBody.innerHTML = '';

    if (!ordens || ordens.length === 0) {
        tableBody.innerHTML = '<tr><td colspan="7" class="text-center">Nenhuma ordem registrada</td></tr>';
        return;
    }

    for (const ordem of ordens) {
        const cliente = allClientes.find(c => String(c.id) === String(ordem.cliente_id));
        const equipamento = allEquipamentos.find(e => String(e.id) === String(ordem.equipamento_id));

        let totalPecas = 0;
        try {
            const pecasOS = await db.getPecasByOrdem(ordem.id);
            if (pecasOS && pecasOS.length > 0) {
                totalPecas = pecasOS.reduce((acc, p) => acc + (Number(p.quantidade || 0) * Number(p.valor_unitario || 0)), 0);
            }
        } catch (err) {
            Logger.error('Erro ao buscar peças para cálculo de total na tabela', err);
        }

        const valorTotalCalculado = Number(ordem.valor_servico || 0) + totalPecas;

        const row = document.createElement('tr');

        const tipo = equipamento?.tipo ? `[${equipamento.tipo}] ` : '';
        const infoEquipamento = equipamento 
            ? `${tipo}${equipamento.marca || ''} ${equipamento.modelo || ''}`.trim()
            : 'N/A';

        row.innerHTML = `
            <td>${ordem.numero_os}</td>
            <td>${cliente?.nome || 'N/A'}</td>
            <td>${infoEquipamento}</td>
            <td>
            ...
            <td>${formatCurrency(valorTotalCalculado)}</td>
            <td>
                <button class="btn btn-small btn-primary" onclick="viewOrderDetails('${ordem.id}')">
                    Ver
                </button>
                <button class="btn btn-small btn-danger" onclick="deleteOrdenServico('${ordem.id}')">
                    Deletar
                </button>
            </td>
        `;
...
function setupEventListeners() {
    ...
    if (clientSelect) {
        clientSelect.addEventListener('change', (e) => {
            ...
            if (selectedClientId) {
                const clientEquipments = allEquipamentos.filter(
                    eq => String(eq.cliente_id) === String(selectedClientId)
                );
                
                populateEquipmentDropdown(clientEquipments);

                const isStillValid = clientEquipments.some(eq => String(eq.id) === String(currentEquipId));
                if (isStillValid && equipmentSelect) {
                    equipmentSelect.value = currentEquipId;
                }
            } else {
                populateEquipmentDropdown(allEquipamentos);
            }
    ...
    if (equipmentSelect) {
        equipmentSelect.addEventListener('change', (e) => {
            const selectedEquipId = e.target.value;

            if (selectedEquipId) {
                const selectedEquip = allEquipamentos.find(eq => String(eq.id) === String(selectedEquipId));

                if (selectedEquip) {
                    populateClientDropdown(allClientes, selectedEquip.cliente_id);

                    const clientEquipments = allEquipamentos.filter(
                        eq => String(eq.cliente_id) === String(selectedEquip.cliente_id)
                    );
                    populateEquipmentDropdown(clientEquipments, selectedEquip.id);
                }
            } else {
...
async function openEditOrderModal(ordemId) {
    ...
    try {
        ...
        populateClientDropdown(allClientes, ordem.cliente_id);
        const clientEquipments = allEquipamentos.filter(
            eq => String(eq.cliente_id) === String(ordem.cliente_id)
        );
        populateEquipmentDropdown(clientEquipments, ordem.equipamento_id);
...
async function filterOrders() {
    ...
    await renderOrdensTable(filtered);
}
...
async function saveOrdenServico(e) {
    ...
    const valorMaoDeObra = parseFloat(document.getElementById('orderValue').value) || 0;

    const ordemData = {
        cliente_id: document.getElementById('orderClient').value,
        equipamento_id: document.getElementById('orderEquipment').value,
        status: document.getElementById('orderStatus').value,
        data_recebimento: document.getElementById('orderDate').value,
        problema_relatado: document.getElementById('orderProblem').value,
        diagnostico_tecnico: document.getElementById('orderDiagnosis').value,
        servicos_realizados: document.getElementById('orderServices').value,
        valor_servico: valorMaoDeObra,
        tecnico_responsavel: document.getElementById('orderTechnician').value
    };
    ...
    try {
        ...
        alert(orderId ? 'Ordem atualizada!' : 'Ordem criada!');
...
function addPartToOrderBuffer() {
    ...
    if (!pecaId) return alert('Selecione uma peça.');
...
function removePartFromOrderBuffer(partKey) {
    ...
    orderPartsBuffer = orderPartsBuffer.filter(p => String(p.id || p.tempId) !== String(partKey));
    renderOrderPartsEditor();
}
...
function renderOrderPartsEditor() {
    ...
    if (orderPartsBuffer.length === 0) {
        tbody.innerHTML = '<tr><td colspan="5" class="text-center">Nenhuma peça adicionada</td></tr>';
        return;
    }
...
async function removeImageFromOrderBuffer(imageKey) {
    ...
    orderImagesBuffer = orderImagesBuffer.filter(i => String(i.id || i.tempId) !== String(imageKey));
    renderOrderImagesEditor();
}
...
function renderOrderImagesEditor() {
    ...
    preview.innerHTML = '';
    if (orderImagesBuffer.length === 0) return;
...
async function syncOrderParts(ordemId) {
    ...
    for (const part of orderPartsBuffer) {
        ...
        if (insertedParts.has(signature)) continue;
...
async function syncOrderImages(ordemId, equipamentoId) {
    ...
    for (const img of orderImagesBuffer) {
        if (img.existing || !img.url) continue;
...
async function deleteOrdenServico(ordemId) {
    if (!confirm('Tem certeza que deseja deletar esta ordem?')) return;
...
async function generateOrderPdf() {
    try {
        ...
        [...imagensDaOs, ...imagensDoEquipamento].forEach(img => {
            if (img?.id)
                imagensMap.set(String(img.id), img);
        });
